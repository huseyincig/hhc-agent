 ...
 */
function normalizedJobPayload(job) {
  const holder =
...
/**
 * @param {import('node:child_process').ChildProcess|undefined|null} child @param {string} [platform] @param {boolean} [force]
 */
/**
...
/**
 * @param {Record<string, unknown>} job @param {{maxStdoutBytes?: number, maxStderrBytes?: number}} [options]
 */
export async function executeShellJob(job, options = {}) {
  ...
  const cwd = String(payload.cwd || job.cwd || layout.root);
  const requestedTimeout = Number(payload.timeout_seconds ?? job.timeout_seconds ?? 30);
  const timeoutSeconds = Math.max(
    1,
    Math.min(Number.isFinite(requestedTimeout) ? requestedTimeout : 30, cfg.maxTimeoutSeconds)
  );
  const maxStdout = options.maxStdoutBytes || cfg.maxStdoutBytes;
  const maxStderr = options.maxStderrBytes || cfg.maxStderrBytes;
